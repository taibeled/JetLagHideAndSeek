...
| 18  | **Deferred upstream.** Installed MapLibre React Native `10.4.2` serializes and replaces the complete GeoJSON shape. Official latest `v11.2.1` docs still expose whole-`data` replacement plus read/cluster methods for `GeoJSONSource`, not cross-platform per-feature mutation, and v11 requires the React Native New Architecture. Revisit if upstream adds incremental updates or profiling justifies a native fork. |
| 19  | **Complete.** Generated Tokyo metadata bundles its bbox, center, and compact mask-hole paths. The default map resolves those paths directly instead of traversing the `175 KB` fixture and recalculating ring winding, without duplicating the boundary coordinates in the bundle.                                                                                                                                      |
| 20  | **Complete.** Persisted non-bundled boundaries keep their cache timestamp in memory. Entries older than `30` days, including legacy timestamp-free entries, are returned immediately and refreshed from Overpass in the background with per-relation in-flight deduplication. Failed refreshes retain the usable stale copy.                                                                                            |
