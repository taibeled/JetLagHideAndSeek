...
- The smoke flow handles Expo dev-client first-run prompts conditionally, then asserts the map has loaded the default `Tokyo 23 Wards` label and the main settings row is targetable. Android CI can spend more than 15 seconds on the cold `Bundling ...` screen after opening the dev-client URL, so the shared app-state wait allows up to 120 seconds. Android may reach the map with only the floating `Open bottom sheet` button exposed in the native hierarchy, so flows tap that button before looking for drawer rows. The map controls are icon-only, so Maestro should not assert old visible copy such as `Fit Tokyo 23 Wards` or `Locate me`.
...
- The default-state Maestro flows call `clearState`, then open the Expo dev-client URL from `MAESTRO_DEV_CLIENT_URL`. iOS dismisses the development-menu intro and closes the dev menu before app assertions; Android waits directly for the app after the dev-client URL because `disableOnboarding=1` skips the prompt while the first bundle can still be compiling. `scripts/e2e-maestro-stack.mjs` sets that URL to `10.0.2.2` on Linux/Android CI and `127.0.0.1` elsewhere; override `E2E_DEV_CLIENT_HOST` for local Android runs on macOS. This avoids depending on a plain Android `launchApp` for Expo dev-client startup while still preventing persisted AsyncStorage setup from leaking into default assertions. Add separate persistence-specific flows when testing relaunch behavior.
